...
class _BatchTransferProcessingScreenState
  ...
  void _navigateToReceipt(BatchTransferEntity response) {
    ...
    // Telemetry: long-flow batch outcome + settlement.
    ...
    AnalyticsService.instance
        .trackBatchTransferOutcome(outcome: outcome, flow: 'long');
    AnalyticsService.instance.trackBatchTransferSettled(status: outcome);

    final receiptData = buildBatchReceiptData(
      response,
  ...
  void _showErrorAndRetry(String message) {
    ...
    AnalyticsService.instance.trackBatchTransferOutcome(
        outcome: isNetworkError ? 'network_error' : 'failed', flow: 'long');
