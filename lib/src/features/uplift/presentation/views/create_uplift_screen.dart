...
class _CreateUpliftScreenState extends State<CreateUpliftScreen> {
  ...
  void initState() {
    ...
    if (e != null) {
      ...
      if (e.hasApplicationDeadline())
        _appDeadline = e.applicationDeadline.toDateTime();
      if (e.hasFundingDeadline())
        _fundingDeadline = e.fundingDeadline.toDateTime();
      _gallery.addAll(e.gallery);
      final eq = UpEquityTerms.fromMetadata(e.metadata);
      if (eq.enabled) {
        _equityEnabled = true;
        _investmentType =
            eq.investmentType.isEmpty ? 'equity' : eq.investmentType;
        if (eq.offeredPct > 0)
          _equityPct.text = (eq.offeredPct / 100)
              .toStringAsFixed(eq.offeredPct % 100 == 0 ? 0 : 2);
        if (eq.valuation > 0)
          _valuation.text = (eq.valuation / 100).toStringAsFixed(0);
  ...
  Future<void> _submit({bool asDraft = false}) async {
    ...
    try {
      ...
      Get.snackbar(
        ...
        asDraft
            ? '${fund.title} saved as a draft — publish it when ready'
            : '${fund.title} is now accepting applications',
        backgroundColor: kUpCard,
        colorText: Colors.white,
        snackPosition: SnackPosition.BOTTOM,
      );
    } catch (e) {
      Get.snackbar(_isEdit ? 'Could not save changes' : 'Could not open fund',
          upFriendlyError(e),
          backgroundColor: kUpError,
          colorText: Colors.white,
          snackPosition: SnackPosition.BOTTOM);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title:
            Text('${_isEdit ? 'Edit fund' : 'Open a fund'} · ${_steps[_step]}'),
      ),
      ...
      bottomNavigationBar: UpBottomNav(
        ...
        nextLabel: _step < _lastStep
            ? 'Continue'
            : (_isEdit ? 'Save changes' : 'Publish fund'),
  ...
  List<Widget> _basicsStep() => [
        _stepHeader('Fund basics',
            'Name your call and describe the kind of business you want to back.'),
            ...
            for (final c in const [
              ...
              ChoiceChip(
                ...
                labelStyle: TextStyle(
                    color:
                        _category.text == c ? Colors.white : kUpTextSecondary,
                    fontSize: 12),
                backgroundColor: kUpCard,
  ...
  List<Widget> _fundingStep() => [
        _stepHeader('Funding shape',
            'Set how much each business can get and how many you\'ll fund.'),
        _field(_cap, 'Max per business (₦)',
            keyboard: TextInputType.number, validator: _num),
        _field(_count, 'How many businesses',
            keyboard: TextInputType.number, validator: _int),
        _datePicker('Application deadline', _appDeadline,
            (d) => setState(() => _appDeadline = d)),
        _datePicker('Funding deadline', _fundingDeadline,
            (d) => setState(() => _fundingDeadline = d)),
      ];

  List<Widget> _dealStep() => [
        _stepHeader('Deal terms',
            'Optionally structure the equity or investment you take in return.'),
        _equitySection(),
      ];
  ...
  List<Widget> _reviewStep() {
    ...
    return [
      _stepHeader('Review',
          'Confirm the details, then publish — or save it as a draft.'),
      UpCard(
        child: Column(
          ...
          children: [
            ...
            _reviewRow('Max per business',
                cap == null ? '—' : upNaira((cap * 100).round(), 'NGN')),
            _reviewRow('Businesses', _count.text.trim()),
            if (_appDeadline != null)
              _reviewRow('Applications close',
                  _appDeadline!.toLocal().toString().split(' ').first),
            if (_fundingDeadline != null)
              _reviewRow('Funding deadline',
                  _fundingDeadline!.toLocal().toString().split(' ').first),
            _reviewRow(
                'Deal terms',
                _equityEnabled
                    ? '${upInvestmentTypeLabel(_investmentType)}${_equityPct.text.trim().isNotEmpty ? ' · ${_equityPct.text.trim()}%' : ''}'
                    : 'None'),
            _reviewRow('Media',
                _gallery.isEmpty ? 'None' : '${_gallery.length} image(s)'),
          ],
        ),
      ),
      if (!_isEdit) ...[
        ...
        Center(
          child: TextButton.icon(
            ...
            icon: const Icon(Icons.save_outlined,
                color: kUpTextSecondary, size: 18),
            label: const Text('Save as draft instead',
  ...
  Widget _equitySection() {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(
            color: kUpPrimary.withOpacity(_equityEnabled ? 0.4 : 0.12)),
      ),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          SwitchListTile(
            ...
            title: const Text('Structure equity / deal terms',
                style: TextStyle(
                    color: Colors.white, fontWeight: FontWeight.w600)),
            subtitle: const Text(
                'Optional — define what you take in return for funding',
                style: TextStyle(color: kUpTextSecondary, fontSize: 12)),
          ),
          if (_equityEnabled) ...[
            const SizedBox(height: 8),
            ...
            Row(children: [
              Expanded(
                  child: _plainField(_equityPct, 'Equity % (per business)',
                      keyboard: TextInputType.number)),
              const SizedBox(width: 10),
              Expanded(
                  child: _plainField(_valuation, 'Valuation ₦ (optional)',
                      keyboard: TextInputType.number)),
            ]),
            const SizedBox(height: 10),
            _plainField(_equityTerms, 'Deal terms / notes (optional)',
                maxLines: 3),
          ],
        ],
      ),
