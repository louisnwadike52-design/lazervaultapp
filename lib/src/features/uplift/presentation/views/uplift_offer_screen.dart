...
class _UpliftOfferScreenState extends State<UpliftOfferScreen> {
  ...
  void initState() {
    ...
    final seedAmountKobo = a.hasCounter && a.counterAmount.toInt() > 0
        ? a.counterAmount.toInt()
        : a.requestedAmount.toInt();
    ...
    if (seedEquity > 0)
      _equityPct.text =
          (seedEquity / 100).toStringAsFixed(seedEquity % 100 == 0 ? 0 : 2);
  ...
  int get _approvedKobo =>
      ((double.tryParse(_approved.text.trim()) ?? 0) * 100).round();
  int get _milestoneSumKobo => _rows.fold(
      0,
      (s, r) =>
          s + ((double.tryParse(r.amount.text.trim()) ?? 0) * 100).round());
  ...
  void _autoSplit() {
    ...
    var remainder =
        _approvedKobo - base * n; // give the remainder to the first row
    for (var i = 0; i < n; i++) {
  ...
  Future<void> _submit() async {
    ...
    if (approved > widget.capKobo) {
      _snack('Max is ${upNaira(widget.capKobo, widget.currency)} per business',
          kUpError);
      return;
    }
    ...
    if (_milestoneSumKobo != approved) {
      _snack(
          'Milestone amounts must add up to ${upNaira(approved, widget.currency)}',
          kUpError);
      return;
    }
    ...
    for (var i = 0; i < _rows.length; i++) {
      ...
      milestones.add(up.UpliftMilestonePlanInput(
        ...
        amount:
            Int64(((double.tryParse(r.amount.text.trim()) ?? 0) * 100).round()),
      ));
    }
  ...
  Widget _feeRow(String k, String v, {bool bold = false, bool warn = false}) =>
      Padding(
        ...
        child:
          ...
          Text(k,
              style: TextStyle(
                  color: warn ? kUpWarning : kUpTextSecondary, fontSize: 12)),
          Text(v,
              style: TextStyle(
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: UpGradientScaffoldBody(
        ...
        children: [
          Text(
              'Cap: up to ${upNaira(widget.capKobo, widget.currency)} · requested ${upNaira(widget.application.requestedAmount.toInt(), widget.currency)}',
          ...
          if (_fee != null && _fee!.feeEnabled && _fee!.fee.toInt() > 0) ...[
            ...
            Container(
              ...
              child: Column(children: [
                _feeRow('Gross disbursement',
                    upNaira(_fee!.gross.toInt(), widget.currency)),
                _feeRow(
                    'Platform fee${_fee!.feeType == 'percentage' && _fee!.feePercentageBps > 0 ? ' (${(_fee!.feePercentageBps / 100).toStringAsFixed(2)}%)' : ''}',
                    '- ${upNaira(_fee!.fee.toInt(), widget.currency)}',
                    warn: true),
                const Divider(color: kUpDivider, height: 14),
                _feeRow('Business receives',
                    upNaira(_fee!.net.toInt(), widget.currency),
                    bold: true),
              ]),
            ),
          ],
          ...
          Container(
            ...
            child: Row(
                mainAxisAlignment: MainAxisAlignment.spaceBetween,
                children: [
                  Text('Milestones total',
                      style:
                          TextStyle(color: balanced ? kUpSuccess : kUpWarning)),
                  Text(
                      '${upNaira(sum, widget.currency)} / ${upNaira(approved, widget.currency)}',
                      style: TextStyle(
                          color: balanced ? kUpSuccess : kUpWarning,
