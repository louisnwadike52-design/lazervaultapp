...
class UpliftMilestonesScreen extends StatefulWidget {
  const UpliftMilestonesScreen(
      {required this.applicationId, required this.isFunder, super.key});
...
class _UpliftMilestonesScreenState extends State<UpliftMilestonesScreen>
  ...
  Future<void> _reject(up.UpliftMilestoneMessage m) async {
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: TextField(
          ...
          decoration: const InputDecoration(
              labelText: 'Reason',
              labelStyle: TextStyle(color: kUpTextSecondary)),
        ),
        actions: [
          TextButton(
          ...
          TextButton(
              ...
              child: const Text('Reject', style: TextStyle(color: kUpError))),
        ],
      ),
    );
    ...
    try {
      await _repo.reviewMilestone(
          milestoneId: m.id, approve: false, reason: reason);
      await _load();
    } catch (e) {
      _err(e);
    }
        ...
        children: [
          Text(a.businessName,
              style: const TextStyle(
                  ...
                  fontSize: 18,
                  fontWeight: FontWeight.w700)),
          Text('Approved ${upNaira(a.approvedAmount.toInt(), a.currency)}',
              style: const TextStyle(color: kUpTextSecondary)),
          const SizedBox(height: 16),
  ...
  Widget _milestoneTile(up.UpliftMilestoneMessage m) {
    final label = m.status.name
        .replaceFirst('UPLIFT_MILESTONE_STATUS_', '')
        .toLowerCase()
        .replaceAll('_', ' ');
    final released =
        m.status == up.UpliftMilestoneStatus.UPLIFT_MILESTONE_STATUS_RELEASED;
    final canSubmit = !widget.isFunder &&
        (m.status == up.UpliftMilestoneStatus.UPLIFT_MILESTONE_STATUS_PLANNED ||
            m.status ==
                up.UpliftMilestoneStatus.UPLIFT_MILESTONE_STATUS_REJECTED);
    final canReview = widget.isFunder &&
        m.status ==
            up.UpliftMilestoneStatus.UPLIFT_MILESTONE_STATUS_EVIDENCE_SUBMITTED;
    return UpCard(
      ...
      child: Column(
        ...
        children: [
          Row(children: [
            Container(
              ...
              decoration: BoxDecoration(
                  color: released ? kUpSuccess : kUpDivider,
          ...
          const SizedBox(height: 6),
          Text(upNaira(m.amount.toInt(), _app?.currency ?? 'NGN'),
              style: const TextStyle(
                  color: kUpPrimary, fontWeight: FontWeight.w700)),
          if (m.rejectReason.isNotEmpty) ...[
            const SizedBox(height: 4),
            Text('Rejected: ${m.rejectReason}',
          ...
          if (m.evidence.isNotEmpty) ...[
            ...
            SizedBox(
              ...
              child: ListView.separated(
                ...
                itemBuilder: (_, i) {
                  final url = m.evidence[i];
                  // A PDF rendered through Image.network fails to decode, and
                  // the old errorBuilder returned an EMPTY box — so a document
                  // submitted as proof disappeared entirely and the funder
                  // approved or rejected the milestone believing no evidence
                  // had been sent.
                  if (upIsPdfUrl(url)) {
                    return InkWell(
                  ...
                  return ClipRRect(
                    ...
                    child: Image.network(url,
                        width: 72,
                        fit: BoxFit.cover,
                        // Still never silently empty: a failed image shows a
                        // placeholder so the funder knows something IS there.
              ...
              child: Row(
                ...
                children: [
                  Row(children: const [
                    ...
                    SizedBox(width: 6),
                    Text('Funds released',
                        style: TextStyle(color: kUpSuccess, fontSize: 12)),
                  ]),
                  if (m.releaseId.isNotEmpty)
                    TextButton.icon(
                      onPressed: () => _openReceipt(m.releaseId),
                      icon: const Icon(Icons.receipt_long,
                          size: 16, color: kUpPrimary),
                      label: const Text('View receipt',
                          style: TextStyle(color: kUpPrimary)),
                    ),
                ],
              ),
            ),
          ...
          if (canReview)
            Row(mainAxisAlignment: MainAxisAlignment.end, children: [
              TextButton(
                  onPressed: () => _reject(m),
                  child:
                      const Text('Reject', style: TextStyle(color: kUpError))),
              ElevatedButton(
                  style: ElevatedButton.styleFrom(
