...
''';
  ...
  void _linkAccount(String code) {
    ...
    // Shared connection-fee notice (generic — no exact amount, we're on a Mono
    // subscription), then link. Charge (default 0, split at settlement) applies
    // server-side on a genuinely new link.
    linkBankWithFee(
      context: context,
      doLink: (token, txnId) async => cubit.linkAccount(
