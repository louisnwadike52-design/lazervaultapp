...
class InternetBillPdfService {
  ...
  /// Generate a professional receipt PDF for an internet bill payment
  static Future<File> generateReceipt({
    required InternetPaymentEntity payment,
  }) async {
    await _loadFonts();
    final pdf = pw.Document();
    final logo = await _loadLogo();
    final generatedDate = _displayDateFormat.format(DateTime.now());
    final currencySymbol = _currencySymbolFor('NGN');
    final receiptNumber =
        'INT-${payment.reference.length > 12 ? payment.reference.substring(0, 12) : payment.reference}';
        ...
        build: (pw.Context context) {
          return pw.Column(
            ...
            children: [
              ...
              _buildInternetDetails(payment: payment),
    ...
    final fileName =
        'internet_bill_receipt_${payment.reference.replaceAll(RegExp(r'[^a-zA-Z0-9]'), '_')}.pdf';
  ...
  static pw.Widget _buildSummaryTable({
    ...
    return pw.Column(
      children: [
        ...
        _buildSummaryRow('Type', 'Internet Bill Payment'),
      ],
    );
  }
  ...
  static pw.Widget _buildInternetDetails({
    required InternetPaymentEntity payment,
  }) {
    return pw.Column(
      crossAxisAlignment: pw.CrossAxisAlignment.start,
          ...
          child: pw.Column(
            children: [
              ...
              if (payment.newBalance > 0)
                _buildDetailRow(
                  'New Balance',
                  'NGN ${_currencyFormat.format(payment.newBalance)}',
                ),
            ],
          ),
        ),
      ...
      children: [
        ...
        pw.Container(
          ...
          child: pw.Text(
            ...
            'This document is a confirmation of an internet bill payment processed through the LazerVault platform. '
  ...
  static Future<void> shareReceipt({
    ...
    try {
      ...
      await SharePlus.instance.share(ShareParams(
        ...
        text:
            'Internet Bill Payment Receipt - NGN $amount to $provider (Account: ${payment.customerNumber})',
        subject: 'LazerVault Internet Bill Payment Receipt',
      ));
    } catch (e) {
