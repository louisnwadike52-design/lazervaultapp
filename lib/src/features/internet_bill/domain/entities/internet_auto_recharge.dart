...
import 'internet_beneficiary.dart';

/// Internet auto-recharge ("Rollover") schedule. Mirrors `DataAutoRecharge`
/// but package-locked: every run re-buys the SAME internet plan
/// (`packageId` + `planName`) so users don't get surprise pricing when
/// ISPs retire packages.
class InternetAutoRecharge extends Equatable {
  final String id;
  final String userId;
  ...
  factory InternetAutoRecharge.fromProto(pb.InternetAutoRecharge proto) {
    ...
    return InternetAutoRecharge(
      ...
      beneficiary: proto.hasBeneficiary()
          ? InternetBeneficiary.fromProto(proto.beneficiary)
          : null,
    );
  }
  ...
  List<Object?> get props =>
      [id, accountNumber, packageId, amount, frequency, status];
}
