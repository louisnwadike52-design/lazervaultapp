...
class AccountsServiceClient extends $grpc.Client {
  ...
  /// Get account by provider reference (internal service-to-service only — no HTTP route).
  /// Used by passive-deposit webhooks (ClearBank, Stripe Treasury, Stitch, etc.) to resolve
  /// the owning virtual account from the external provider's identifier.
  $grpc.ResponseFuture<$0.GetAccountByProviderRefResponse>
      getAccountByProviderRef(
    $0.GetAccountByProviderRefRequest request, {
    $grpc.CallOptions? options,
  }) {
  ...
  /// Get total ledger balance across all active accounts (internal service-to-service only).
  /// Used by banking-service daily reconciliation to compare ledger total vs pooled bank balance.
  ...
  /// Read-only lookup of a transaction by its full reference string. Used by
  /// background reconcilers (supervisors, rollback processors) in other
  /// services to verify whether a previously-attempted debit or credit
  /// actually committed — WITHOUT the side effect of re-attempting it via
  /// an idempotent RPC. This is the safe verification path that prevents
  /// double-charges in the corner case where a service retries with the
  /// same idempotency key but the original request never reached this
  /// service in the first place.
  /// INTER-SERVICE ONLY — deliberately has NO google.api.http annotation.
  ///
  /// Background reconcilers use this to ask "did this debit commit?" before
  /// retrying, which is why it is also JWT-exempt in the allowlist in
  /// cmd/main.go. Both of those are correct for a service-to-service probe.
  ///
  /// It previously carried `get: "/api/v1/transactions/by-reference/{reference}"`,
  /// which put it on the grpc-gateway mux that core-gateway registers publicly.
  /// Nothing in that path checks ownership — the service method is
  /// FindByReference(reference), with no user id — so any signed-in user could
  /// read any other user's transaction: amount, currency, description, status,
  /// balance_after, account id. References are not secret; they travel in
  /// receipts, shared payment links and counterparty notifications.
  ///
  /// Removing the annotation is the actual repair. core-gateway also blocks the
  /// path (interceptInternalOnlyRoutes) as defence in depth, so a future
  /// gateway that registers this mux differently cannot re-expose it.
  ///
  /// Do not add an http option here. If this ever needs to be user-callable,
  /// it needs an owner check first — scope the query to the caller's accounts,
  /// do not just re-expose the route.
  ...
  /// GetLedgerEntriesByReference returns ALL ledger legs for a single logical
  /// money movement keyed by its canonical reference / idempotency key. Unlike
  /// LookupTransactionByReference (single row), an internal transfer writes two
  /// legs under DERIVED references ("IDEM-XFER-{key}-FROM" debit /
  /// "IDEM-XFER-{key}-TO" credit; legacy: "{ref}" + "{ref}-CR"), so this returns
  /// the full double-entry set. Read-only; used by core-payments' admin
  /// transfer-detail assembly to render the debit+credit audit.
  $grpc.ResponseFuture<$0.GetLedgerEntriesByReferenceResponse>
      getLedgerEntriesByReference(
    $0.GetLedgerEntriesByReferenceRequest request, {
    $grpc.CallOptions? options,
  }) {
    return $createUnaryCall(_$getLedgerEntriesByReference, request,
        options: options);
  }
  ...
  /// Full per-lock settlement timeline (principal lock, interest payouts,
  /// maturity payout) read from lock_funds_settlements — the lock-id-precise
  /// history the app renders alongside wallet transactions. gRPC-only
  /// (called by the commerce-gateway lock-funds proxy).
  $grpc.ResponseFuture<$0.GetLockFundsSettlementsResponse>
      getLockFundsSettlements(
    $0.GetLockFundsSettlementsRequest request, {
    $grpc.CallOptions? options,
  }) {
    return $createUnaryCall(_$getLockFundsSettlements, request,
        options: options);
  }

  /// Renew an active lock by extending unlock_date for a fresh
  /// term (in days). Re-pays upfront interest if the plan supports
  /// it. Caller can renew at any point before maturity; matured
  /// locks should create a fresh lock instead.
  ...
  /// Cancel an active lock. Returns principal (less penalty) to
  /// the source account; flips status to 'cancelled'. Reason is
  /// stored for ops audit.
  ...
  /// Admin force-mature: skip the unlock_date check + skip the
  /// user-id ownership check (caller is an admin acting on behalf
  /// of a user). Captures admin_user_id + reason for the audit
  /// trail. Returns principal in full to the source account — no
  /// penalty because this is an ops override (e.g. plan
  /// discontinued, regulatory ask). Distinct RPC so the user-side
  /// CancelLockFunds stays scoped and we can require admin role
  /// on this one.
  $grpc.ResponseFuture<$0.AdminForceMatureLockFundsResponse>
      adminForceMatureLockFunds(
    $0.AdminForceMatureLockFundsRequest request, {
    $grpc.CallOptions? options,
  }) {
  ...
  /// Admin force-cancel: same as CancelLockFunds but bypasses the
  /// user-id ownership check and applies the configured penalty.
  /// For cases where ops needs to abort a lock the user can't
  /// reach (account frozen, suspended kyc, etc.).
  $grpc.ResponseFuture<$0.AdminForceCancelLockFundsResponse>
      adminForceCancelLockFunds(
    $0.AdminForceCancelLockFundsRequest request, {
    $grpc.CallOptions? options,
  }) {
  ...
  /// Request a FOREIGN-CURRENCY (USD/GBP/EUR) virtual account via Fincra.
  /// Issuance is ASYNC: this submits the compliance request (document URLs +
  /// full KYC profile) and returns the pending request id; account details
  /// land on the user's currency account once Fincra approves + issues
  /// (poll worker / webhook). The request is validated for completeness
  /// server-side — an incomplete submission is rejected, never padded.
  $grpc.ResponseFuture<$0.RequestForeignVirtualAccountResponse>
      requestForeignVirtualAccount(
    $0.RequestForeignVirtualAccountRequest request, {
    $grpc.CallOptions? options,
  }) {
    return $createUnaryCall(_$requestForeignVirtualAccount, request,
        options: options);
  }

  /// Fetch the FCY provisioning state for the caller's account in a currency
  /// (pending/active/failed + account details once issued).
  $grpc.ResponseFuture<$0.GetForeignVirtualAccountStatusResponse>
      getForeignVirtualAccountStatus(
    $0.GetForeignVirtualAccountStatusRequest request, {
    $grpc.CallOptions? options,
  }) {
    return $createUnaryCall(_$getForeignVirtualAccountStatus, request,
        options: options);
  }
  ...
  /// Read current spending usage (limits + today's/this month's spend) for the
  /// account. Powers the "Today's usage" bar in the app's account limits tab.
  $grpc.ResponseFuture<$0.GetSpendingUsageResponse> getSpendingUsage(
    $0.GetSpendingUsageRequest request, {
    $grpc.CallOptions? options,
  }) {
    return $createUnaryCall(_$getSpendingUsage, request, options: options);
  }
  ...
  /// Admin: List the latest wallet reconciliation row per
  /// (wallet_code, currency). Optional filters narrow the result set
  /// for dashboards that only show one wallet at a time.
  ...
  /// Admin: Trigger an immediate reconciliation pass. Idempotent —
  /// writes one row per (wallet_code, currency) with the current
  /// drift. Returns count of rows persisted.
  ...
  /// Admin: REVENUE balance snapshots over a time window. Drives the
  /// dashboard sparkline so ops can see trends before the 24h growth
  /// alarm fires.
  ...
  /// Admin: list the recent operational-alert dispatch history.
  /// Filters by status / source / dedup_key are all optional; results
  ...
  /// Slice 10h: on-call ack / silence controls. Ack is informational
  /// (audit-side). Silence extends the dedup window for a given
  /// dedup_key past the default 1h so repeated triggers during a
  /// known incident don't keep emailing.
  ...
  $grpc.ResponseFuture<$0.AdminUnsilenceOpsAlertResponse>
    ...
    $0.AdminUnsilenceOpsAlertRequest request, {
    $grpc.CallOptions? options,
  }) {
  ...
  /// Slice 10j: full detail (incl. body_html) for a single
  /// ops_alerts row. The list RPC deliberately omits body_html for
  /// payload size; only this detail call returns it.
  ...
  /// Admin: list the BCP-47 locales the platform supports for
  /// PiggyVault content (and other localized surfaces). Read from
  /// system_settings key `platform.supported_locales`. Used by the
  /// admin dashboard locale dropdown.
  $grpc.ResponseFuture<$0.AdminListSupportedLocalesResponse>
      adminListSupportedLocales(
    $0.AdminListSupportedLocalesRequest request, {
    $grpc.CallOptions? options,
  }) {
  ...
  /// Admin: read the full i18n bundle for a PiggyVault config so the
  /// dashboard's locale-aware editor can pre-fill per-locale strings.
  ...
  /// Admin: write the i18n bundle for one locale of a PiggyVault
  /// config. Replaces just that locale's strings — other locales in
  ...
  static final _$getAccountByProviderRef = $grpc.ClientMethod<
      ...
      $0.GetAccountByProviderRefResponse.fromBuffer);
  static final _$getTotalLedgerBalance = $grpc.ClientMethod<
      ...
      $0.GetTotalLedgerBalanceResponse.fromBuffer);
  ...
  static final _$lookupTransactionByReference = $grpc.ClientMethod<
      ...
      $0.LookupTransactionByReferenceResponse.fromBuffer);
  static final _$getLedgerEntriesByReference = $grpc.ClientMethod<
          $0.GetLedgerEntriesByReferenceRequest,
          $0.GetLedgerEntriesByReferenceResponse>(
      '/accounts.AccountsService/GetLedgerEntriesByReference',
      ($0.GetLedgerEntriesByReferenceRequest value) => value.writeToBuffer(),
      $0.GetLedgerEntriesByReferenceResponse.fromBuffer);
  ...
  static final _$getLockFundsSettlements = $grpc.ClientMethod<
          $0.GetLockFundsSettlementsRequest,
          $0.GetLockFundsSettlementsResponse>(
      '/accounts.AccountsService/GetLockFundsSettlements',
      ($0.GetLockFundsSettlementsRequest value) => value.writeToBuffer(),
      $0.GetLockFundsSettlementsResponse.fromBuffer);
  static final _$renewLockFunds =
      $grpc.ClientMethod<$0.RenewLockFundsRequest, $0.RenewLockFundsResponse>(
          ...
          $0.RenewLockFundsResponse.fromBuffer);
  static final _$cancelLockFunds =
      $grpc.ClientMethod<$0.CancelLockFundsRequest, $0.CancelLockFundsResponse>(
          ...
          $0.CancelLockFundsResponse.fromBuffer);
  static final _$adminForceMatureLockFunds = $grpc.ClientMethod<
      ...
      $0.AdminForceMatureLockFundsResponse.fromBuffer);
  static final _$adminForceCancelLockFunds = $grpc.ClientMethod<
      ...
      $0.AdminForceCancelLockFundsResponse.fromBuffer);
  ...
  static final _$requestForeignVirtualAccount = $grpc.ClientMethod<
          $0.RequestForeignVirtualAccountRequest,
          $0.RequestForeignVirtualAccountResponse>(
      '/accounts.AccountsService/RequestForeignVirtualAccount',
      ($0.RequestForeignVirtualAccountRequest value) => value.writeToBuffer(),
      $0.RequestForeignVirtualAccountResponse.fromBuffer);
  static final _$getForeignVirtualAccountStatus = $grpc.ClientMethod<
          $0.GetForeignVirtualAccountStatusRequest,
          $0.GetForeignVirtualAccountStatusResponse>(
      '/accounts.AccountsService/GetForeignVirtualAccountStatus',
      ($0.GetForeignVirtualAccountStatusRequest value) => value.writeToBuffer(),
      $0.GetForeignVirtualAccountStatusResponse.fromBuffer);
  ...
  static final _$getSpendingUsage = $grpc.ClientMethod<
          $0.GetSpendingUsageRequest, $0.GetSpendingUsageResponse>(
      '/accounts.AccountsService/GetSpendingUsage',
      ($0.GetSpendingUsageRequest value) => value.writeToBuffer(),
      $0.GetSpendingUsageResponse.fromBuffer);
  ...
  static final _$adminListWalletReconciliations = $grpc.ClientMethod<
      ...
      $0.AdminListWalletReconciliationsResponse.fromBuffer);
  static final _$adminRunWalletReconciliation = $grpc.ClientMethod<
      ...
      $0.AdminRunWalletReconciliationResponse.fromBuffer);
  static final _$adminGetRevenueBalanceSnapshots = $grpc.ClientMethod<
      ...
      ($0.AdminGetRevenueBalanceSnapshotsRequest value) =>
          value.writeToBuffer(),
      $0.AdminGetRevenueBalanceSnapshotsResponse.fromBuffer);
  static final _$adminListOpsAlerts = $grpc.ClientMethod<
      ...
      $0.AdminListOpsAlertsResponse.fromBuffer);
  static final _$adminAckOpsAlert = $grpc.ClientMethod<
      ...
      $0.AdminAckOpsAlertResponse.fromBuffer);
  static final _$adminSilenceOpsAlert = $grpc.ClientMethod<
      ...
      $0.AdminSilenceOpsAlertResponse.fromBuffer);
  static final _$adminUnsilenceOpsAlert = $grpc.ClientMethod<
      ...
      $0.AdminUnsilenceOpsAlertResponse.fromBuffer);
  ...
  static final _$adminListSupportedLocales = $grpc.ClientMethod<
      ...
      $0.AdminListSupportedLocalesResponse.fromBuffer);
  static final _$adminGetPiggyVaultConfigI18n = $grpc.ClientMethod<
      ...
      $0.AdminGetPiggyVaultConfigI18nResponse.fromBuffer);
  static final _$adminUpsertPiggyVaultConfigI18n = $grpc.ClientMethod<
      ...
      ($0.AdminUpsertPiggyVaultConfigI18nRequest value) =>
          value.writeToBuffer(),
      $0.AdminUpsertPiggyVaultConfigI18nResponse.fromBuffer);
...
abstract class AccountsServiceBase extends $grpc.Service {
  ...
  AccountsServiceBase() {
    ...
    $addMethod($grpc.ServiceMethod<$0.LookupTransactionByReferenceRequest,
        ...
        ($0.LookupTransactionByReferenceResponse value) =>
            value.writeToBuffer()));
    $addMethod($grpc.ServiceMethod<$0.GetLedgerEntriesByReferenceRequest,
            $0.GetLedgerEntriesByReferenceResponse>(
        'GetLedgerEntriesByReference',
        getLedgerEntriesByReference_Pre,
        false,
        false,
        ($core.List<$core.int> value) =>
            $0.GetLedgerEntriesByReferenceRequest.fromBuffer(value),
        ($0.GetLedgerEntriesByReferenceResponse value) =>
            value.writeToBuffer()));
    ...
    $addMethod($grpc.ServiceMethod<$0.GetLockFundsSettlementsRequest,
            $0.GetLockFundsSettlementsResponse>(
        'GetLockFundsSettlements',
        getLockFundsSettlements_Pre,
        false,
        false,
        ($core.List<$core.int> value) =>
            $0.GetLockFundsSettlementsRequest.fromBuffer(value),
        ($0.GetLockFundsSettlementsResponse value) => value.writeToBuffer()));
    ...
    $addMethod($grpc.ServiceMethod<$0.RequestForeignVirtualAccountRequest,
            $0.RequestForeignVirtualAccountResponse>(
        'RequestForeignVirtualAccount',
        requestForeignVirtualAccount_Pre,
        false,
        false,
        ($core.List<$core.int> value) =>
            $0.RequestForeignVirtualAccountRequest.fromBuffer(value),
        ($0.RequestForeignVirtualAccountResponse value) =>
            value.writeToBuffer()));
    $addMethod($grpc.ServiceMethod<$0.GetForeignVirtualAccountStatusRequest,
            $0.GetForeignVirtualAccountStatusResponse>(
        'GetForeignVirtualAccountStatus',
        getForeignVirtualAccountStatus_Pre,
        false,
        false,
        ($core.List<$core.int> value) =>
            $0.GetForeignVirtualAccountStatusRequest.fromBuffer(value),
        ($0.GetForeignVirtualAccountStatusResponse value) =>
            value.writeToBuffer()));
    $addMethod(
    ...
    $addMethod($grpc.ServiceMethod<$0.GetSpendingUsageRequest,
            $0.GetSpendingUsageResponse>(
        'GetSpendingUsage',
        getSpendingUsage_Pre,
        false,
        false,
        ($core.List<$core.int> value) =>
            $0.GetSpendingUsageRequest.fromBuffer(value),
        ($0.GetSpendingUsageResponse value) => value.writeToBuffer()));
  ...
  $async.Future<$0.LookupTransactionByReferenceResponse>
      lookupTransactionByReference_Pre(
          $grpc.ServiceCall $call,
          $async.Future<$0.LookupTransactionByReferenceRequest>
              $request) async {
  ...
  $async.Future<$0.GetLedgerEntriesByReferenceResponse>
      getLedgerEntriesByReference_Pre($grpc.ServiceCall $call,
          $async.Future<$0.GetLedgerEntriesByReferenceRequest> $request) async {
    return getLedgerEntriesByReference($call, await $request);
  }

  $async.Future<$0.GetLedgerEntriesByReferenceResponse>
      getLedgerEntriesByReference($grpc.ServiceCall call,
          $0.GetLedgerEntriesByReferenceRequest request);
  ...
  $async.Future<$0.GetLockFundsSettlementsResponse> getLockFundsSettlements_Pre(
      $grpc.ServiceCall $call,
      $async.Future<$0.GetLockFundsSettlementsRequest> $request) async {
    return getLockFundsSettlements($call, await $request);
  }

  $async.Future<$0.GetLockFundsSettlementsResponse> getLockFundsSettlements(
      $grpc.ServiceCall call, $0.GetLockFundsSettlementsRequest request);
  ...
  $async.Future<$0.RequestForeignVirtualAccountResponse>
      requestForeignVirtualAccount_Pre(
          $grpc.ServiceCall $call,
          $async.Future<$0.RequestForeignVirtualAccountRequest>
              $request) async {
    return requestForeignVirtualAccount($call, await $request);
  }

  $async.Future<$0.RequestForeignVirtualAccountResponse>
      requestForeignVirtualAccount($grpc.ServiceCall call,
          $0.RequestForeignVirtualAccountRequest request);

  $async.Future<$0.GetForeignVirtualAccountStatusResponse>
      getForeignVirtualAccountStatus_Pre(
          $grpc.ServiceCall $call,
          $async.Future<$0.GetForeignVirtualAccountStatusRequest>
              $request) async {
    return getForeignVirtualAccountStatus($call, await $request);
  }

  $async.Future<$0.GetForeignVirtualAccountStatusResponse>
      getForeignVirtualAccountStatus($grpc.ServiceCall call,
          $0.GetForeignVirtualAccountStatusRequest request);
  ...
  $async.Future<$0.GetSpendingUsageResponse> getSpendingUsage_Pre(
      $grpc.ServiceCall $call,
      $async.Future<$0.GetSpendingUsageRequest> $request) async {
    return getSpendingUsage($call, await $request);
  }

  $async.Future<$0.GetSpendingUsageResponse> getSpendingUsage(
      $grpc.ServiceCall call, $0.GetSpendingUsageRequest request);
  ...
  $async.Future<$0.AdminRunWalletReconciliationResponse>
      adminRunWalletReconciliation_Pre(
          $grpc.ServiceCall $call,
          $async.Future<$0.AdminRunWalletReconciliationRequest>
              $request) async {
  ...
  $async.Future<$0.AdminGetPiggyVaultConfigI18nResponse>
      adminGetPiggyVaultConfigI18n_Pre(
          $grpc.ServiceCall $call,
          $async.Future<$0.AdminGetPiggyVaultConfigI18nRequest>
              $request) async {
