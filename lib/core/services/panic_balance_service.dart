...
class PanicBalanceService extends ChangeNotifier {
  ...
  Future<void> init() async {
    ...
    _visible = p.getBool(_kVisible) ?? false;
    _activeLabel = p.getString(_kActiveLabel);
  ...
  Future<void> _persist() async {
    ...
    await p.setBool(_kVisible, _visible);
    if (_activeLabel != null) {
      await p.setString(_kActiveLabel, _activeLabel!);
    } else {
      await p.remove(_kActiveLabel);
    }
    await p.setString(
        _kPresets, jsonEncode(_presets.map((e) => e.toJson()).toList()));
  }
